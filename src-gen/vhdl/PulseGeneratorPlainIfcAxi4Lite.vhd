...
begin
	...
	ReadMatch <= PreMatchReadControlReg
			or PreMatchReadPulsePeriodReg
			or PreMatchReadPulseWidthReg;
	
	WriteMatch <= PreMatchWriteControlReg
			or PreMatchWritePulsePeriodReg
			or PreMatchWritePulseWidthReg;
	
	ReadAck <= PreReadAckControlReg
			or PreReadAckPulsePeriodReg
			or PreReadAckPulseWidthReg;
	
	WriteAck <= PreWriteAckControlReg
			or PreWriteAckPulsePeriodReg
			or PreWriteAckPulseWidthReg;
	...
	begin
		if (Rst = '1') then
			...
			if (WriteDiffControlReg = '1') then
				if (WriteByteEnable(0) = '1') then WRegOperation(1 downto 0) <= WriteData(1 downto 0); end if;
			end if;
		end if;
	end process;
	...
	PreMatchReadPulsePeriodRegProcess : process (ReadAddress, ReadAddressMatch)
	begin
		if (unsigned(ReadAddress) >= unsigned(PULSEPERIODREG_ADDRESS) and
			unsigned(ReadAddress) <= unsigned(PULSEPERIODREG_ADDRESS) + 3) then
			PreMatchReadPulsePeriodReg <= ReadAddressMatch;
		else
			PreMatchReadPulsePeriodReg <= '0';
		end if;
	end process;
	
	PreMatchWritePulsePeriodRegProcess : process (WriteAddress, WriteAddrMatch)
	begin
		if (unsigned(WriteAddress) >= unsigned(PULSEPERIODREG_ADDRESS) and
			unsigned(WriteAddress) <= unsigned(PULSEPERIODREG_ADDRESS) + 3) then
			PreMatchWritePulsePeriodReg <= WriteAddrMatch;
		else
			PreMatchWritePulsePeriodReg <= '0';
		end if;
	end process;
	
	WriteDiffPulsePeriodRegProcess : process (WriteAddress, Write, PreWriteAckPulsePeriodReg)
	begin
		if (unsigned(WriteAddress) >= unsigned(PULSEPERIODREG_ADDRESS) and
			unsigned(WriteAddress) <= unsigned(PULSEPERIODREG_ADDRESS) + 3) then
	...
	ReadDiffPulsePeriodRegProcess : process (ReadAddress, Read, PreReadAckPulsePeriodReg)
	begin
		if (unsigned(ReadAddress) >= unsigned(PULSEPERIODREG_ADDRESS) and
			unsigned(ReadAddress) <= unsigned(PULSEPERIODREG_ADDRESS) + 3) then
	...
	PreMatchReadPulseWidthRegProcess : process (ReadAddress, ReadAddressMatch)
	begin
		if (unsigned(ReadAddress) >= unsigned(PULSEWIDTHREG_ADDRESS) and
			unsigned(ReadAddress) <= unsigned(PULSEWIDTHREG_ADDRESS) + 3) then
			PreMatchReadPulseWidthReg <= ReadAddressMatch;
		else
			PreMatchReadPulseWidthReg <= '0';
		end if;
	end process;
	
	PreMatchWritePulseWidthRegProcess : process (WriteAddress, WriteAddrMatch)
	begin
		if (unsigned(WriteAddress) >= unsigned(PULSEWIDTHREG_ADDRESS) and
			unsigned(WriteAddress) <= unsigned(PULSEWIDTHREG_ADDRESS) + 3) then
			PreMatchWritePulseWidthReg <= WriteAddrMatch;
		else
			PreMatchWritePulseWidthReg <= '0';
		end if;
	end process;
	
	WriteDiffPulseWidthRegProcess : process (WriteAddress, Write, PreWriteAckPulseWidthReg)
	begin
		if (unsigned(WriteAddress) >= unsigned(PULSEWIDTHREG_ADDRESS) and
			unsigned(WriteAddress) <= unsigned(PULSEWIDTHREG_ADDRESS) + 3) then
	...
	ReadDiffPulseWidthRegProcess : process (ReadAddress, Read, PreReadAckPulseWidthReg)
	begin
		if (unsigned(ReadAddress) >= unsigned(PULSEWIDTHREG_ADDRESS) and
			unsigned(ReadAddress) <= unsigned(PULSEWIDTHREG_ADDRESS) + 3) then
